...
`lose(...)`: Llamada al sistema POSIX para liberar el descriptor de archivo local. Al utilizar UDP, esta llamada no genera tráfico de red (no envía paquetes FIN/ACK), simplemente destruye el endpoint a nivel del sistema operativo local.
